...
class CanvasDnD (tk.Frame):
  ...
  def down (self, event):
    print("Click on ", event.widget.itemcget (tk.CURRENT, "text"))
    self.loc =1
    self.dragged =0
  ...
  def up (self, event):
    ...
    if (self.target ==event.widget.find_withtag (tk.CURRENT)):
      print("Select ", event.widget.itemcget (tk.CURRENT, "text"))
    else:
